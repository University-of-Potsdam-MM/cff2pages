...
The testing routine for `cff2pages` ensures that `.cff` or `.json` files are correctly converted into their respective HTML or Markdown outputs.
...
1. **Iterate Through Fixture Files**
   All `.cff` and `.json` files in the `fixtures` directory are collected and tested one by one.

2. **HTML Conversion Subtest**
   For each `.cff` and `.json` file:
...
3. **Markdown Conversion Subtest**
   ...
   - The `.cff` and `.json` file is converted to a Markdown output.
...
4. **Subtest Separation**
   Each `.cff` `.json` file is run as a separate subtest for both formats (`html` and `md`), allowing clear test reports and isolation of failures.

Fixtures Used
-------------

Currently, the test suite includes **three `.cff` files** and **one `.json` file** in the `fixtures/` directory:
...
- **`current_codemeta.json`**  `see here <_static/citation/codemeta.html>`_
  A structured metadata example used to validate `.json` support.
...
- **Fixtures**: Input `.cff` or `.json` files to test various metadata combinations.
...
- For a file `example.cff` or `example.json`, the test will compare:
